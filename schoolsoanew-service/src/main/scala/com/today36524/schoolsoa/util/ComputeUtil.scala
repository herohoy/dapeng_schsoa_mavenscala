...
import com.today36524.schoolsoa.bean.{Clazz, Grade, School, Student}
import com.today36524.schoolsoa.dao.{ClassDao, GradeDao, StudentDao}
...
object ComputeUtil {
  ...
  private def studentAchList(c:Clazz = null,g:Grade = null,s:School = null) = {
    ...
    if(s!=null)
    {
      for(sgrs <- GradeDao.findGradesBySchool(s.id))
        for(scls <- ClassDao.findClassesByGrade(sgrs.id))
          stuList ++= StudentDao.findStudentRowsByClass(scls.id)
    }
    stuList.toList
  }
  ...
  def top20ofGrades(g:Grade = null,s:School = null) = {
    ...
    if(s!=null)
      {
        sgn.append((if(g!=null) "以及" else "") + "学校"+s.name+"中的各个年级：")
        val gl = GradeDao.findGradesBySchool(s.id)
        for(i <- gl.indices)
          sgn.append(if(i!=0) "," else "" +gl(i).getName + "年级")
      }
